function ShowGreeneSteelArcade(){
	var arcadeNavWin = Ti.UI.createWindow({
		width:640/2,
		height:960/2,
		bottom:-960/2,
		navBarHidden:true
	...
	var arcadeWin = Ti.UI.createWindow({
		width:640/2,
		height:960/2,
	...
	arcadeWin.add(testView);
	
	var lblTitle = Ti.UI.createLabel({
		width:'auto',
		height:'auto',
		font:{fontSize:18,fontFamily:'Big Caslon'},
		text:'Green Steel Arcade',
		color:'#fff'
	})
	...
	arcadeWin.setLeftNavButton(btnClose);
	arcadeWin.setRightNavButton(lblTitle);
	
	var arcadeNav = Ti.UI.iPhone.createNavigationGroup({
		window:arcadeWin,
	});
	arcadeNavWin.add(arcadeNav);
	...
	xpIcon.addEventListener('touchend',function(){
		...
		if(!checkIfProductPurchased('com.tigerdog.lifequest.minigame.pack1')){
			alert("You haven't purchased the Mini Game Pack 1.  Please purchase this Mini Game Pack in order to play!");
		} else {
			StartDungeonCrawlGame();
			//StartShooterGame()
			//StartBubblePopGame();
			//Ti.API.info('start shooter game')
		}
